...
"""
Script 08 (v2): Integrar Dataset Manual (Phase 2) - VERSÃO CORRIGIDA

Objetivo:
  - Varrer todas as edições (Janeiro a Junho 2025)
  - Encontrar pares PDF + MD (procura AMBOS: .md e .corrigido.md)
  - Copiar para pasta estruturada (train/val/test)
  - Gerar relatório de integração

Mudanças v2:
  - Procura por .md E .corrigido.md (AMBOS os padrões!)
  - Esperado: ~1.179 pares (não apenas 216)

Uso:
  python scripts/08_integrar_dataset_manual_v2.py
"""
...
def encontrar_pares_pdf_md(dataset_path):
    ...
    for mes_dir in sorted(dataset_path.iterdir()):
        ...
        for edicao_dir in sorted(mes_dir.iterdir()):
            ...
            # Procurar pasta "divididos" (onde estão os .md)
            ...
            # ============================================================
            # PROCURAR AMBOS OS PADRÕES
            # ============================================================

            # Procurar .corrigido.md (Março)
            md_corrigido = list(divididos_dir.glob("*.corrigido.md"))

            # Procurar .md simples (Abril, Maio, Junho)
            md_simples = list(divididos_dir.glob("*.pagina*.md"))
            # Filtrar para não pegar .corrigido.md novamente
            md_simples = [f for f in md_simples if ".corrigido.md" not in f.name]

            # Combinar ambas as listas
            md_files = md_corrigido + md_simples

            # ============================================================

            for md_path in md_files:
                # Extrair nome base
                # Pode ser: DA01P01.pagina1.corrigido.md ou DA01P01.pagina1.md
                nome_arquivo = md_path.stem  # Remove última extensão
                if nome_arquivo.endswith(".corrigido"):
...
def copiar_pares(pares, dest_images, dest_annot, split_name):
    ...
    for i, par in enumerate(pares, 1):
        ...
        # Nome destino: DA01P01.pagina1 (sem extensão)
...
def gerar_relatorio(train, val, test):
    ...
    with open(PROJECT_ROOT / "relatorios" / "integracao_fase2_v2.txt", 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("FASE 2 (v2): INTEGRAÇÃO DE DATASET MANUAL - VERSÃO CORRIGIDA\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write("\n\nNOTA IMPORTANTE (v2):\n")
        f.write("-" * 80 + "\n")
        f.write("Esta versão procura AMBOS os padrões de gabarito:\n")
        f.write("  - .corrigido.md (Março)\n")
        f.write("  - .md simples (Abril, Maio, Junho)\n")
        f.write("  - Janeiro e Fevereiro não têm gabaritos\n")
        f.write(f"Total esperado: ~1.179 pares\n")

    print(f"\n📊 Relatórios gerados:")
    print(f"  ✓ data_splits.json")
    print(f"  ✓ dataset_stats.json")
    print(f"  ✓ integracao_fase2_v2.txt")
...
def main():
    print("=" * 80)
    print("FASE 2 (v2): INTEGRAR DATASET MANUAL - VERSÃO CORRIGIDA")
    print("=" * 80)
    ...
    # 2. Encontrar pares
    print("\n2️⃣  Encontrando pares PDF + MD (AMBOS os padrões)...")
    ...
    print("\n" + "=" * 80)
    print("✅ FASE 2 (v2) COMPLETA!")
    print("=" * 80)
    print(f"\n📁 Dataset integrado em: {DATA_PROCESSED}")
    print(f"\n📊 Próximo passo: Fazer commit no Git e iniciar Fase 3 (Baseline VLM)")
